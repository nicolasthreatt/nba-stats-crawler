...
def parse(table: str, stat_key: str, stat_type: str, player: Player = None, teams: dict = None):

    table_type = TableType.PLAYER.name if player else TableType.TEAM.name

    # TODO: WRITE IN A CLEANER WAY
    if stat_key == "Overall":
        table_header_row, table_column_offset = getStatColumnType('Opponent Shooting ' + stat_type, table_type)
    else:
        table_header_row, table_column_offset = getStatColumnType('Opponent Shooting ' + stat_key, table_type)
